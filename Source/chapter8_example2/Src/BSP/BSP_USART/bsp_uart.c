...
uint32_t console_putchar(char c)
{
    ...
    // Send character
    LL_USART_TransmitData8(CONSOLE_USART, (uint8_t)c);
...
uint32_t console_getchar(char *c)
{
    ...
    // Check if data is available (RXNE flag is set)
    if (!LL_USART_IsActiveFlag_RXNE(CONSOLE_USART))
        return ERROR_FAIL;

    // Read received data
    *c = (char)LL_USART_ReceiveData8(CONSOLE_USART);
