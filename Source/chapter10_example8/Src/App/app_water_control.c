...
void static app_water_control_fsm_transition(uint32_t ui32TargetState, uint32_t ui32CurrentState)
{
	...
	for ( ui32TargetCount = 0; ui32TargetCount < APP_WATER_CONTROL_MAX_PARENT; ui32TargetCount++)
	{
	...
	for ( ui32CurrentCount = 0; ui32CurrentCount < APP_WATER_CONTROL_MAX_PARENT; ui32CurrentCount++)
	{
...
void app_water_control_update(void)
{
	...
	if (ERROR_OK == fifo_get(&app_control_evt_fifo, &evt))
	{
		...
		app_water_control_fsm_dispatch(&evt);
	}
}
